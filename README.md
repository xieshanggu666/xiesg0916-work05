...
| 双盲仲裁 | 负责人发起后系统为双方各建隔离标注：OPEN 期间掩膜仅本侧可见可改（服务端强制），双方提交后自动计算 XOR 差异连通域，仲裁者逐区域裁定 A/B，合并生成正式掩膜版本（`source=arbitration`）直接进复核 |
...
POST   /arbitrations                     发起双盲仲裁（负责人 + 双方标注者 + 仲裁者）
GET    /arbitrations/{id}                仲裁详情（进度、差异区域、裁定记录）
GET    /arbitrations/{id}/mask?side=a    查看某侧掩膜（OPEN 期间仅本侧可见）
POST   /arbitrations/{id}/submit         一侧提交（提交后冻结）
